...
export default function Dashboard() {
    ...
    return (
        < template.Home MainContent={() => (<MainContent solvedProblems={solvedProblems.current} tournaments={pastTournaments.current} user={user.current} pendingSubmissions={pendingSubmissions.current}/>)} SSelected={'dashboard'} promise={promise} />
    ) 
}

function MainContent({solvedProblems, tournaments, user, pendingSubmissions}) {
        ...
        <>
            <div className='section'>
                ...
                <template.StaticTable id="solved_problems" headers={['Problem Title', 'Exp']} width={[7,1]} data={pendingSubmissions.map(
                    (problem) => ([<a href={'problems/' + problem.id}>{problem.id}</a>, problem.xp]))} />
            </div>
            <div className='section'>
